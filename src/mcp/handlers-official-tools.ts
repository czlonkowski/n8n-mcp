...
export async function callOfficialTool(
  ...
  try {
    ...
    if (result.isError || (data as any)?.ok === false) {
      return { success: false, action: label, officialTool: tool, code: 'OFFICIAL_MCP_ERROR', error: (data as any)?.message ?? (data as any)?.error ?? 'n8n returned an error', officialError: data };
    }
