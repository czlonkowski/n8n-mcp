...
- **Graceful degradation for older n8n.** Group support is discovered from the write itself rather than a version probe: when n8n rejects `nodeGroups` as an unknown property the workflow is sent again without it, and when it rejects the group `description` (added in n8n 2.32) the groups are sent without descriptions. Both cases warn instead of failing. Degradation requires n8n to identify the offending property, which its request validator reports — a rejection naming nothing is surfaced as an error rather than guessed at, since attributing it to groups could disable them for an instance that supports them. In practice only an explicitly authored grouping can reach that path, because an instance without the field never returns it to begin with.
