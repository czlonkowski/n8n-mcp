...
- **The stdio read-buffer limit is raised to 64 MB, above the SDK's new 10 MB default.** 1.30.0 introduced a cap on stdio where there was none: a single inbound JSON-RPC message over the limit makes the transport emit an error and close. That failure is not a tool error the client can report — the session dies — and stdio is the npx channel, where there is no telemetry and the user's machine has the version cached. Accepting the default would therefore turn a `n8n_update_full_workflow` body that worked yesterday into a dead session today, on the channel with the slowest path to a fix. stdio's peer is the user's own MCP client over a local pipe rather than an untrusted network caller, so the case for a tight bound is weaker here than on the HTTP transport, which keeps its `10mb` JSON body limit and answers oversized bodies with an error rather than a disconnect. 64 MB keeps a backstop against a stream that never terminates a message while clearing any realistic workflow payload, pinned data included. The limit applies only to messages the server reads; tool result size is unaffected.
- **The stdio transport now logs its errors.** Nothing was attached to `onerror`, so a transport-level failure — the new buffer cap among them — ended the session with no diagnostic. The SDK chains a handler set before `connect()` ahead of its own, so the log line does not displace the SDK's error propagation.
