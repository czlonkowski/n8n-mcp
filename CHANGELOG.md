...
- **"Available in MCP" is only ever enabled with `exposeToMcp: true`.** The consent flow only ever turns the setting on, never off, and n8n-mcp never disables it implicitly. Turning it off stays a deliberate act: `n8n_update_partial_workflow`'s `updateSettings` operation (or a create / full update) with `availableInMCP: false`, or the n8n UI.
