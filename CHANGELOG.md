...
- **The Docker builder no longer compiles against a stale n8n.** Its minimal build-dependency set pinned `n8n-workflow@^2.4.2`, which resolves through the package's `latest` dist-tag — that tag lags the release n8n ships (2.16.1 versus the 2.31.3 in `package.json`), so the image was built against older type definitions and failed on `agentSelector`. The pin is now exact and `scripts/update-n8n-deps.js` keeps it in step with `package.json`.
