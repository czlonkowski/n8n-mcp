...
- **A malformed node payload is reported as a validation error in `n8n_update_partial_workflow` and `validate_workflow`, not as a TypeError** ([#1092](https://github.com/czlonkowski/n8n-mcp/issues/1092)). 2.84.1 closed this for `n8n_create_workflow` and `n8n_update_full_workflow`; the same symptom was still reachable in the tool that receives the most writes and in the offline validator an agent calls most often. In the diff engine, an operation's `node`, `updates` and `connections` arrive as `z.any()` — they have to, because the operation contracts are looser than n8n's node schema — and each was dereferenced before anything checked it, so the caller got `Diff engine error: node.type.includes is not a function` against operation index `-1` instead of a message naming the field and the operation. Shape checks now run first for `addNode` (object, string `name`, string `type`, `[x, y]` `position` — the one required field `applyAddNode` does not default), `updateNode` (`updates` must be an object of field paths) and `replaceConnections` (`connections` must be an object). The batch-reordering pass that hoists an `addNode` above an earlier connection operation read `node.name` before any validator ran, so a `null` node bypassed the new check entirely; it now treats a non-object payload as referenced by nothing. In `validate_workflow`, a non-object entry, a non-string `type` or a node with no `parameters` threw several passes in, and the single try/catch around the run turned it into `Workflow validation failed: Cannot read properties of null (reading 'type')` with every other check skipped — those three shapes are now reported per node, while an incomplete draft (no `id`, `name`, `typeVersion` or `position`) still receives its normal feedback rather than a shape rejection.
